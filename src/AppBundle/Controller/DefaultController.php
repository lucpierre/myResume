        ...
        if($request->isMethod('POST')) 
        {
            ...
            if ($form->isValid())
            {
                $message = (new Swift_Message())
                    ->setSubject($request->request->get('object'))
                    ->setFrom([ $request->request->get('email')=> $request->request->get('author')])
                    ->setTo('lucquiaudpierre@gmail.com')
                    ->setBody($request->request->get('body'))
                ;

                $sendingMail = $this->get('mailer')->send($message);

                if (sendingMail != 0)
                {
                    $this->get('session')->getFlashBag()->add('mail-status', 'Message envoyé !');
